...
const perjalananDinasControllers = async (req, res) => {
  const { nama, page = 1, limit = 10 } = req.body;
  ...
  try {
    ...
    SELECT * FROM (
      SELECT
        ...
        ROW_NUMBER() OVER (ORDER BY aih.invoice_date DESC, aih.invoice_num, pay.check_date) AS row_num,
      ...
      FROM
        ...
        LEFT JOIN (
          SELECT invoice_id, SUM(amount) AS pph_amt 
          FROM ap_invoice_distributions_all 
          WHERE line_type_lookup_code = 'AWT' 
          GROUP BY invoice_id
        ) pph ON aih.invoice_id = pph.invoice_id
        LEFT JOIN (
          SELECT invoice_id, SUM(amount) AS prepay_amt 
          FROM ap_invoice_distributions_all 
          WHERE line_type_lookup_code = 'PREPAY' 
          GROUP BY invoice_id
        ) prepay ON aih.invoice_id = prepay.invoice_id  
      WHERE
    ...
    const json = await executeOracleQuery(connection, query, params);

    res.json({
      success: true,
      page: Number(page),
      limit: Number(limit),
      total: json.data.length > 0 ? json.data[0].TOTAL_COUNT : 0,
      data: json.data,
    });
  } catch (error) {
